...
- The Driver mix range can create a homogeneous population by setting both bounds to one level, or a uniform mixture across any inclusive range. It defaults to levels 2 through 4, so an ordinary population contains cautious, normal, and assertive drivers rather than one temperament. A car draws its level when its cached spawn profile is created and keeps that character throughout its run; changing the range during a run therefore affects only newly arriving cars, while restarting redraws the whole population.
...
The Findings section below the graph reports seven sweeps against a single rush-hour scenario, which is also the default state of the page: 20-second green, 15 cars per minute, 70 percent compliance, a 50-metre striped zone, a 6-metre intended distance, a 50 km/h limit, and a mixed population spanning driver levels 2 to 4. Each point averages 20 seeded runs of five minutes, and each finding carries its own chart of the Lane B over Lane A throughput ratio. Lane B moves about 30 percent more cars in that scenario. Compliance pays back proportionally with no minimum threshold, the striped zone needs roughly 40 metres, green phases stop mattering past about twelve seconds, higher speed limits help up to about 60 km/h, driver temperament barely changes the relative advantage while dominating absolute throughput, and intended distances above six metres add nothing.

Three caveats apply when reading the graph. Throughput is quantised, because each signal cycle discharges a whole number of cars, so the ratio moves in steps rather than smoothly. At low arrival rates Lane B can clear nearly all existing demand, so its curve flattens against a demand ceiling rather than a capacity limit; the default rate of 15 cars per minute is high enough to avoid this. Most importantly, the sweeps vary one parameter at a time around one scenario, and the parameters interact, so other regions of the parameter space can shift or reverse a trend seen here.
