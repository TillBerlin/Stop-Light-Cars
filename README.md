...
- The moving safety distance includes the normal gap, distance closed on the leader during reaction time, and the extra braking distance required when the follower is faster. While a car must stop for the signal, its target is the greater of that safety distance and its lane-specific standstill gap; once it may proceed, it uses the normal moving safety distance.
