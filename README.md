...
The statistics panel can sweep green phase, compliance, striped zone length, Lane B intended distance, arrival rate, speed limit (20 to 100 km/h in steps of 10), and driver aggressiveness. The Findings section below the graph records five hypotheses checked against sweeps of 20 runs per point at five minutes per run, each carrying its headline statistic and a verdict. Two required correction: the advantage peaks at a roughly 15-second green phase rather than growing with longer phases, and the striped zone needs about 40 metres rather than 30 to reach its full effect.

Two artifacts distort naive readings of the graph. At the default arrival rate Lane B frequently clears nearly all existing demand, so its curve flattens against a demand ceiling rather than a capacity limit; comparisons near that ceiling should be repeated at a higher arrival rate. Throughput is also quantised, because each signal cycle discharges a whole number of cars, so the ratio moves in steps such as 8/6 or 6/4 and differences below roughly five percent are not resolvable.
