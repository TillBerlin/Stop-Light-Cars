...
class FakeElement {
  ...
  set innerHTML(value) {
    ...
    if (value.includes('<output>')) this.appendChild(Object.assign(new FakeElement('output'), { tagName: 'output' }));
  }
...
test('default 60-second simulation keeps safety interventions within its regression budget', () => {
  ...
  assert.ok(result.diagnostics.crashes <= 1,
    `observed ${result.diagnostics.crashes} collision corrections`);
  // The current model produces 32 interventions with this deterministic
  // population. Keep a little tolerance while making a sudden increase fail.
  assert.ok(result.diagnostics.emergencyBrakes <= 40,
    `observed ${result.diagnostics.emergencyBrakes} emergency-brake events`);
});

test('no car remains stopped for three seconds behind a gap larger than ten metres', () => {
  const result = simulationResult;
  assert.deepEqual(result.diagnostics.prolongedOpenGaps, []);
});

test('the first four initial cars in both lanes start during the first green phase', () => {
  const result = simulationResult;
  for (const lane of [0, 1]) {
    const starts = result.diagnostics.starts.filter(start => start.lane === lane).slice(0, 4);
...
test('cars crossing on red were committed to crossing during orange', () => {
  const redCrossings = simulationResult.diagnostics.lineCrossings
    .filter(crossing => crossing.phase === 'red');
