...
for (const def of controlDefinitions) {
  ...
  input.addEventListener('input', () => {
    ...
    if (def.key === 'phase' && !state.running) state.phaseRemaining = state.elapsed === 0 ? INITIAL_RED_TIME : settings.phase;
...
let nextCarId = 1;
const state = { running: false, phase: 'red', phaseRemaining: INITIAL_RED_TIME, elapsed: 0, lastFrame: 0, lanes: [] };
...
function reset() {
  ...
  state.running = false; state.phase = 'red'; state.phaseRemaining = INITIAL_RED_TIME; state.elapsed = 0; state.lastFrame = 0;
...
function updateLane(lane, dt) {
  ...
  for (let i = 0; i < lane.cars.length; i++) {
    ...
    // Cars travel toward decreasing positions, so a following car has the
    // larger position. Measure the clear space from its front to the rear of
    // the car ahead. This lets every car whose resting gap is already safe
    // begin its reaction timer at the same time.
    const gap = ahead ? car.position - ahead.position - CAR_LENGTH : Infinity;
